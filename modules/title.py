...
class YTData():
    ...
    def db_set(self, video_id: str, video_title: str, video_author: str) -> None:
        ...
        if db_video is None:
            ...
            # modify video
            cur.execute(f"UPDATE cache SET title=:title WHERE video_id=:video_id", {"title": video_title, 'video_id': video_id})
            cur.execute(f"UPDATE cache SET title=:title WHERE author=:author", {"title": video_title, 'author': video_author})
        con.close()
